...
double du2dx(int i, int j, double **U, double dx, double alpha){
	...
	double summand2 = fabs(U[i][j]+U[i+1][j])*(U[i][j]-U[i+1][j])/4 - fabs(U[i-1][j]+U[i][j])*(U[i-1][j]-U[i][j])/4;
...
double dv2dy(int i, int j, double **V, double dy, double alpha){
	...
	double summand2 = fabs(V[i][j]+V[i][j+1])*(V[i][j]-V[i][j+1])/4 - fabs(V[i][j-1]+V[i][j])*(V[i][j-1]-V[i][j])/4;
...
double duvdy(int i, int j, double **U, double **V, double dy, double alpha){
	...
	double summand2 = fabs(V[i][j]+V[i+1][j])*(U[i][j]-U[i][j+1])/4 - fabs(V[i][j-1]+V[i+1][j-1])*(U[i][j-1]-U[i][j])/4;
	return (summand1+summand2*alpha)/dy;
}
...
double duvdx(int i, int j, double **U, double **V, double dx, double alpha){
	...
	double summand2 = fabs(U[i][j]+U[i][j+1])*(V[i][j]-V[i+1][j])/4 - fabs(U[i-1][j]+U[i-1][j+1])*(V[i-1][j]-V[i][j])/4;
	return (summand1+summand2*alpha)/dx;
}
